...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Process form data
    $caseName = isset($_POST['caseName']) ? $_POST['caseName'] : '';
    $caseNumber = isset($_POST['caseNumber']) ? $_POST['caseNumber'] : '';
    $caseStage = isset($_POST['caseStage']) ? $_POST['caseStage'] : '';
    $dateOpened = isset($_POST['dateOpened']) ? $_POST['dateOpened'] : '';
    $office = isset($_POST['office']) ? $_POST['office'] : '';
    $description = isset($_POST['description']) ? $_POST['description'] : '';
    $statuteOfLimitations = isset($_POST['statuteOfLimitations']) ? $_POST['statuteOfLimitations'] : '';
...
// Check if the deleteCases form is submitted
if (isset($_POST['deleteCases'])) {
    // Get the selected case IDs
    $selectedCases = isset($_POST['selectedCases']) ? $_POST['selectedCases'] : [];

    if (!empty($selectedCases)) {
        // Use a loop to delete each selected case
        foreach ($selectedCases as $caseId) {
            // Delete case from the cases table
            $deleteCaseQuery = "DELETE FROM cases WHERE case_id = ?";
            $stmtDeleteCase = mysqli_prepare($conn, $deleteCaseQuery);
            mysqli_stmt_bind_param($stmtDeleteCase, "i", $caseId);
            mysqli_stmt_execute($stmtDeleteCase);
            mysqli_stmt_close($stmtDeleteCase);

            // Delete associated case_clients records
            $deleteCaseClientsQuery = "DELETE FROM case_clients WHERE caseId = ?";
            $stmtDeleteCaseClients = mysqli_prepare($conn, $deleteCaseClientsQuery);
            mysqli_stmt_bind_param($stmtDeleteCaseClients, "i", $caseId);
            mysqli_stmt_execute($stmtDeleteCaseClients);
            mysqli_stmt_close($stmtDeleteCaseClients);

            // You may need additional logic to handle other related tables
        }

        // Provide feedback to the user
        echo '<div class="alert alert-success" role="alert">Selected cases deleted successfully.</div>';
    } else {
        // No cases selected for deletion
        echo '<div class="alert alert-warning" role="alert">No cases selected for deletion.</div>';
    }
}

    // Fetch cases and associated clients from the database
    $query = "SELECT case_id, caseName, caseNumber, caseStage, dateOpened, office, description, statuteOfLimitations FROM cases WHERE caseStage = 'Close case'";
...
<body>
        ...
        <div id="subbar-navigation" class="clearfix d-print-none d-none d-lg-block">
            <nav class="navbar navbar-expand-lg navbar-light bg-light ps-4 pe-1 py-0 border-bottom test-subbar-nav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item">
                        <a class="pendo-subnav-recent-activity nav-link active" href="cases.php">Open Cases</a>
                    </li>

                    <li class="nav-item">
                        <a class="pendo-subnav-recent-activity nav-link active" href="closed cases.php">Closed Cases</a>
                    </li>
        ...
        <!---------------------------------------------------------- end of nav bar--------------------------------------------------- -->
            ...
            <div class="d-flex align-items-center pb-3">
                <h3 class="my-3 mx-3 me-4 fw-bold"> Closed Cases</h3>
...
<form action="" method="post">
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (!empty($caseId)) {
            ...
            echo '<tr>';
            // echo '<td><input type="checkbox" name="selectedCases[]" value="' . $row['case_id'] . '"></td>';
    ...
    <!-- <button type="submit" class="btn btn-danger" name="deleteCases">Delete Selected Cases</button> -->
    </form>
